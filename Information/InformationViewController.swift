...
class InformationViewController: UIViewController {
    ...
    func checkTC(){
        ...
        if aceptedTC {
            if let image = UIImage(systemName: "checkmark.square") {
                tcButton.setImage(image, for: .normal)
            }
        }
    }
    ...
    @IBAction func tcButtonTapped(_ sender: Any) {
        ...
        if aceptedTC {
            ...
            DataService.shared.setAceptedTC(false)
        }else{
            if let image = UIImage(systemName: "checkmark.square") {
                tcButton.setImage(image, for: .normal)
            }
            DataService.shared.setAceptedTC(true)
        }
    }
